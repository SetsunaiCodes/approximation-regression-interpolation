...
def update_plot(submit_clicks, generate_clicks, clear_clicks, input_x, input_y):
    ...
    if button_id == 'submit-btn' and submit_clicks > 0:
        if input_x is not None and input_y is not None:
            # fügt den neuen Punkt am ende ein (zum sortieren: .sort())
    ...
    if data["X"] and data["Y"]:
        ...
        wert1, wert2 = regress(data)
        fig = px.scatter(x=data["X"], y=data["Y"])
        fig.add_scatter(x=wert1, y=wert2, mode='lines', name='Regression Line')
